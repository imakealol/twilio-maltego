...
def call_forwarding_phone_lookup(account_sid: str, auth_token: str, phone_number: str):
    client = Client(account_sid, auth_token)
    response = client.lookups.v2.phone_numbers(phone_number).fetch(fields='call_forwarding')
...
@registry.register_transform(
    display_name="Check for call forwarding [twilio]", 
    input_entity="maltego.PhoneNumber",
    description='Returns details from Twilio API lookup for call forwarding ($/lookup)',
    settings=[],
    output_entities=["maltego.Unknown"],
    transform_set=twilio_set
    )
class checkCallForwarding(DiscoverableTransform):
    ...
    def create_entities(cls, request: MaltegoMsg, response: MaltegoTransform):
        ...
        async def main():
            ...
            lookup = call_forwarding_phone_lookup(account_sid, auth_token, phone_number)
