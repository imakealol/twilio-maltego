...
def all_phone_lookup(account_sid: str, auth_token: str, phone_number: str):
    client = Client(account_sid, auth_token)
    response = client.lookups.v2.phone_numbers(phone_number).fetch(fields='line_type_intelligence,call_forwarding,caller_name')
...
class checkAll(DiscoverableTransform):
    ...
    def create_entities(cls, request: MaltegoMsg, response: MaltegoTransform):
        ...
        async def main():
            ...
            lookup = all_phone_lookup(account_sid, auth_token, phone_number)
