...
def basic_phone_lookup(account_sid: str, auth_token: str, phone_number: str):
...
@registry.register_transform(
    display_name="Basic phone number validation [twilio]", 
    input_entity="maltego.PhoneNumber",
    description='Validate phone number with Twilio API (free with API key)',
    settings=[],
    output_entities=["maltego.Unknown"],
    transform_set=twilio_set
    )
class validateNumber(DiscoverableTransform):
    ...
    def create_entities(cls, request: MaltegoMsg, response: MaltegoTransform):
        ...
        async def main():
            ...
            lookup = basic_phone_lookup(account_sid, auth_token, phone_number)
